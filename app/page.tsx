...
export default function Home() {
  ...
  useEffect(() => {
    if (!running || paused || inFlight.current) return;
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        if (!response.ok) throw new Error(data.error || 'The decision request failed. Please retry.');
        const answer = data.answers?.[`pirate_${next.tokenId}`];
        if (data.mode === 'live' && answer?.choice && Number.isFinite(answer.confidence)) {
          destination = answer.choice; confidence = answer.confidence; setLiveMode('live'); setLatency(data.elapsedMs);
  ...
  const reset = () => { runEpoch.current += 1; setRunning(false); setPaused(false); setCards(makeCards()); setPlacedCount(0); setDecisionCount(0); setReviewCount(0); setLiveMode('preview'); setError(''); setLatency(null); };
  ...
  return <main className="dock-shell">
    <header className="topbar">
      ...
      <div className="top-actions"><span className={`mode-pill ${liveMode}`}><span className="pulse-dot" />{liveMode === 'live' ? 'Live Jev' : 'Preview mode'}</span><a className="text-link" href="https://venice.ai/lp/jev" target="_blank" rel="noreferrer">How Jev works <ExternalLink size={13} /></a></div>
    </header>
