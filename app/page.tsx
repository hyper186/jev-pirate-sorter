...
import { Anchor, ChevronDown, ExternalLink, Gauge, Info, Pause, Play, RotateCcw, Sparkles, Volume2, VolumeX, Zap } from 'lucide-react';
...
export default function Home() {
  ...
  const [liveMode, setLiveMode] = useState<'preview' | 'live'>('preview');
  const [preset, setPreset] = useState(0);
  const runEpoch = useRef(0);
  const bins = mode === 'traits' ? traitBins.map((label) => ({ id: label, label, description: `Official Pirate Nation Character Type family: ${label}.` })) : orderPresets[preset].bins;
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      let confidence = 0.93;
      try {
        const response = await fetch('/api/decisions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ records: [next], bins }) });
        const data = await response.json();
        const answer = data.answers?.[`pirate_${next.tokenId}`];
        if (answer?.choice) { destination = answer.choice; confidence = answer.confidence ?? 0; setLiveMode(data.mode === 'live' ? 'live' : 'preview'); }
      } catch { setLiveMode('preview'); }
      const accepted = confidence >= 0.85 && bins.some((bin) => bin.id === destination);
  ...
  const reset = () => { runEpoch.current += 1; setRunning(false); setPaused(false); setCards(makeCards()); setPlacedCount(0); setDecisionCount(0); setReviewCount(0); setLiveMode('preview'); };
  const start = () => { if (placedCount === pirates.length) reset(); setRunning(true); setPaused(false); };
  ...
  return <main className="dock-shell">
    <header className="topbar">
      ...
      <div className="top-actions"><span className={`mode-pill ${liveMode}`}><span className="pulse-dot" />{liveMode === 'live' ? 'Live Jev' : 'Preview mode'}</span><button className="icon-btn" onClick={() => setSound(!sound)} aria-label={sound ? 'Mute sound' : 'Enable sound'}>{sound ? <Volume2 size={17} /> : <VolumeX size={17} />}</button><a className="text-link" href="https://venice.ai/lp/jev" target="_blank" rel="noreferrer">How Jev works <ExternalLink size={13} /></a></div>
    </header>
    ...
    <section className="stats-row"><div><span>DECIDED</span><strong>{decisionCount}</strong><small>/ {pirates.length}</small></div><div><span>PLACED</span><strong className="teal">{placedCount}</strong><small>cards</small></div><div><span>IN REVIEW</span><strong className="amber">{reviewCount}</strong><small>uncertain</small></div><div className="stats-note"><Zap size={15} /> One shared state. Many typed decisions. <b>{mode === 'traits' ? 'Official metadata' : 'Human-written criteria'}</b></div></section>
    <section className="sorting-layout"><div className="stage-wrap"><div className="stage-head"><div><span className="section-label">LIVE SORTING FLOOR</span><p>{running ? (paused ? 'Dock paused — decisions are held safely.' : 'Arms are reading the pile…') : 'The whole collection starts here.'}</p></div><div className="arm-status"><i className="arm-light" /> 4 arms online</div></div><div className="stage"><div className="stage-grid" /><div className="glow" /><div className="arm arm-left"><span className="joint one" /><span className="joint two" /><span className="gripper" /></div><div className="arm arm-right"><span className="joint one" /><span className="joint two" /><span className="gripper" /></div><div className="pile-label">UNSORTED PILE <b>{visibleCards.length}</b></div><div className="card-pile">{visibleCards.map((card, index) => <button key={card.tokenId} className={`pirate-card ${card.status} ${selectedId === card.tokenId ? 'selected' : ''}`} onClick={() => setSelectedId(card.tokenId)} style={{ left: `${card.x}px`, top: `${card.y}px`, zIndex: index, transform: `rotate(${card.rotation}deg) ${card.status === 'moving' ? 'translate(90px, -40px) scale(1.04)' : ''}` }}><img src={artStyle === 'illustrated' ? card.images.illustrated : card.images.voxel} alt={card.name} /><span>#{card.tokenId}</span></button>)}</div><div className="dock-floor" /></div><div className="pile-grid">{bins.map((bin) => <div className="destination-pile" key={bin.id}><div className="pile-crest">{bin.id === 'spooky' ? '☾' : bin.id === 'bright' ? '✦' : bin.id === 'captain' ? '⚓' : '◇'}</div><div><span>{bin.label}</span><strong>{cards.filter((card) => card.status === 'placed' && card.destination === bin.id).length}</strong></div><small>{bin.description}</small></div>)}</div></div><aside className="inspector"><div className="inspector-head"><div><span className="section-label">DECISION INSPECTOR</span><h3>Pirate #{selected?.tokenId}</h3></div><Info size={17} /></div>{selected && <><div className="inspector-image"><img src={artStyle === 'illustrated' ? selected.images.illustrated : selected.images.voxel} alt={selected.name} /><span className="id-badge">#{selected.tokenId}</span></div><div className="pirate-name">{selected.name}<a href={selected.source} target="_blank" rel="noreferrer"><ExternalLink size={13} /></a></div><div className="trait-list">{Object.entries(selected.traits).map(([key, value]) => <div key={key}><span>{key}</span><b>{value}</b></div>)}{!selected.characterType && <div><span>Category</span><b className="fallback">Rare fallback</b></div>}</div><div className="decision-box"><div className="decision-line"><span>JEV DESTINATION</span><b>{selected.status === 'unsorted' ? 'Waiting…' : selected.destination || 'Review'}</b></div><div className="confidence"><span>CONFIDENCE</span><strong>{selected.confidence ? `${Math.round(selected.confidence * 100)}%` : '—'}</strong></div><div className="confidence-bar"><i style={{ width: `${(selected.confidence || 0) * 100}%` }} /></div><small>{liveMode === 'live' ? 'Live answer from Venice · jev-latest' : 'Local preview · add VENICE_API_KEY for live Jev'}</small></div></>}</aside></section>
    <footer><span>Artwork from <a href="https://github.com/proofofplay/piratenation-art" target="_blank" rel="noreferrer">Pirate Nation Art</a> · CC0</span><span>Jev returns decisions, not prose. <a href="https://typesafe.ai" target="_blank" rel="noreferrer">TypeSafe AI ↗</a></span></footer>
  </main>;
}
