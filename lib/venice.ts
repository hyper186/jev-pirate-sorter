...
export async function decide(request: Request, keyValue: string | undefined, upstreamFetch: typeof fetch = fetch) {
  ...
  try {
    ...
    if (!upstream.ok) return Response.json({ error: `Venice could not complete the decision (HTTP ${upstream.status}). Check the key and model access, then retry.` }, { status: 502 });
    if (!data?.answers || body.records.some((r: any) => { const a = data.answers[`pirate_${r.tokenId}`]; return !a || typeof a.choice !== 'string' || !Number.isFinite(a.confidence) || a.confidence < 0 || a.confidence > 1; })) return Response.json({ error: 'Venice returned an unexpected answer format. Sorting stopped.' }, { status: 502 });
