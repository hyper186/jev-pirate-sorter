...
type Props = { pirates: PirateRecord[]; bins: Bin[]; run: DockRun | null; delivered: number; progress: number; selected: number; art: 'illustrated' | 'voxel'; onSelect: (id: number) => void; paused: boolean };
...
export default function SortingFloor({ pirates, bins, run, delivered, progress, selected, art, onSelect, paused }: Props) {
  ...
  const grouped = allBins.map(bin => pirates.slice(0, delivered).filter(p => run && destinationFor(run.batch.answers[`pirate_${p.tokenId}`], bins) === bin.id));
  const openIndex = allBins.findIndex(b => b.id === openPile);
  return <div className="sorting-surface">
    <div className="stage-head"><div><span className="section-label">THE SORTING DOCK</span><p aria-live="polite">{pose ? `${paused ? 'Paused · ' : ''}${pose.phase} pirate #${current?.tokenId} → ${allBins[targetIndex]?.label}` : delivered === pirates.length ? 'Manifest complete. Inspect any pile.' : 'One shared pile. Waiting for your orders.'}</p></div><span className="sample-tag">{pirates.length} PFP sample</span></div>
    <div className="floor-scroll"><svg className="sorting-svg" viewBox="0 0 1000 700" aria-label="Pirate sorting dock with two articulated arms and destination piles">
      ...
      <text x="500" y="495" textAnchor="middle" className="svg-label">UNSORTED MANIFEST</text><text x="500" y="527" textAnchor="middle" fill="#90e9d2" fontSize="27">{pirates.length - delivered}</text>
      {allBins.map((bin, index) => { const point = pilePosition(index, allBins.length); const active = !!current && bin.id === target; return <g key={bin.id}>
        <g role="button" tabIndex={0} aria-label={`Open ${bin.label} pile, ${grouped[index].length} pirates`} onClick={() => setOpenPile(openPile === bin.id ? null : bin.id)} onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setOpenPile(bin.id); } }} className="pile-hit">
        ...
        {grouped[index].slice(-4).map((pirate, i) => <Portrait key={pirate.tokenId} pirate={pirate} art={art} point={{x:point.x - 32 + i * 19,y:point.y + 4}} angle={-9 + i * 6} active={selected === pirate.tokenId} onSelect={onSelect} />)}
      </g>; })}
      {pirates.slice(delivered).map((pirate, i) => { const index = delivered + i; if (i === 0 && pose) return null; return <Portrait key={pirate.tokenId} pirate={pirate} art={art} point={sourcePosition(index)} angle={-14 + index * 7 % 28} active={selected === pirate.tokenId} onSelect={onSelect} />; })}
    ...
    <p className="floor-hint">Select a pirate to inspect its decision. Select a pile to browse its crew. On small screens, swipe across the dock.</p>
    {openIndex >= 0 && <section className="pile-browser"><div><h3>{allBins[openIndex].label} · {grouped[openIndex].length} pirates</h3><button onClick={() => setOpenPile(null)} aria-label="Close pile browser">Close</button></div><p>{allBins[openIndex].description}</p><div className="pile-gallery">{grouped[openIndex].map(p => <button key={p.tokenId} onClick={() => onSelect(p.tokenId)}><img src={p.images[art]} alt={p.name} loading="lazy" /><span>#{p.tokenId}</span></button>)}{!grouped[openIndex].length && <p>This pile is empty so far.</p>}</div></section>}
  </div>;
}
